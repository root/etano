...
if (mysql_num_rows($res)) {
	...
	if ($rsrow['field_type']=='field_select' || $rsrow['field_type']=='field_mchecks') {
		$m=array();
		preg_match_all("/\['_lang'\]\[(\d+)\]/",$rsrow['custom_config'],$m);
		$key_ids=array_merge($key_ids,$m[1]);
		$query="DELETE FROM `{$dbtable_prefix}profile_field_accvals` WHERE `fk_pfield_id`=$pfield_id";
		if (!($res=@mysql_query($query))) {trigger_error(mysql_error(),E_USER_ERROR);}
	}
