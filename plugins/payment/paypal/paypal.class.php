									...
									if (number_format($real_prod['price'],2)==number_format($input['mc_gross'],2)) {
										if ($input['test_ipn']!=1 || ($this->config['demo_mode']==1 && $input['test_ipn']==1)) {
											...
											if (empty($real_user['user_id'])) {
												...
												$query="INSERT IGNORE INTO `".USER_ACCOUNTS_TABLE."` SET `".USER_ACCOUNT_USER."`='".$input['payer_email']."',`".USER_ACCOUNT_PASS."`=md5('".$input['pass']."'),`email`='".$input['payer_email']."',`membership`=4,`status`=".ASTAT_ACTIVE;
												if (!($res=@mysql_query($query))) {trigger_error(mysql_error(),E_USER_ERROR);}
