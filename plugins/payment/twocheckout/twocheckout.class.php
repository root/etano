...
// alternate method of figuring out if we already have this customer in the db
						} elseif (!empty($input['x_Email'])) {
							$query="SELECT `fk_user_id`,`_user` as `user` FROM `{$dbtable_prefix}user_profiles` WHERE `f2`='".$input['x_Email']."' OR `f7`='".$input['x_Email']."' OR `f8`='".$input['x_Email']."' LIMIT 1";
							if (!($res=@mysql_query($query))) {trigger_error(mysql_error(),E_USER_ERROR);}
							if (mysql_num_rows($res)) {
								$real_user=mysql_fetch_assoc($res);
							}
						...
						if (mysql_num_rows($res)) {
							...
							if (number_format($real_prod['price'],2)==number_format($input['x_amount'],2)) {
								if (strcasecmp($input['demo'],'Y')!=0 || ($this->config['demo_mode']==1 && strcasecmp($input['demo'],'Y')==0)) {
									...
									$this->check_fraud($input);
									// insert the new subscription
									$query="INSERT INTO `{$dbtable_prefix}payments` SET `gateway`='twocheckout',`is_subscr`=0,`is_active`=0,`fk_subscr_id`=0,`gw_txn`='".$input['x_trans_id']."',`name`='".$input['card_holder_name']."',`country`='".$input['x_Country']."',`state`='".$input['x_State']."',`city`='".$input['x_City']."',`zip`='".$input['x_Zip']."',`street_address`='".$input['x_Address']."',`email`='".$input['x_Email']."',`phone`='".$input['x_Phone']."',`amount_paid`='".$input['x_amount']."',`is_suspect`=".((int)$this->is_fraud).",`suspect_reason`='".addslashes($this->fraud_reason)."'";
									...
									if (empty($real_user['user_id'])) {
										...
										$now=gmdate('YmdHis');
										$query="INSERT INTO `{$dbtable_prefix}user_profiles` SET `fk_user_id`='".$real_user['user_id']."',`_user`='".$real_user['user']."',`last_changed`='$now',`date_added`='$now',`status`=".STAT_APPROVED.",`f1`='".$input['card_holder_name']."',`f2`='".$input['x_Email']."',`score`='".$input['x_amount']."'";
										if ($this->is_fraud) {
										...
										if (!($res=@mysql_query($query))) {trigger_error(mysql_error(),E_USER_ERROR);}
										$real_user['user']=get_user_by_userid($real_user['user_id']);
									}
									// if he bought Etano, create a new site
									if (in_array(ETANO_PROD_ID,$prods)) {
										$query="INSERT INTO `{$dbtable_prefix}user_sites` SET `fk_user_id`=".$real_user['user_id'].",`active`=".((int)(!$this->is_fraud));
										if (!($res=@mysql_query($query))) {trigger_error(mysql_error(),E_USER_ERROR);}
										...
										$query="UPDATE `{$dbtable_prefix}user_sites` SET `license`='".$input['license']."',`license_md5`=md5('".$input['license']."') WHERE `site_id`=".$input['site_id'];
										if (!($res=@mysql_query($query))) {trigger_error(mysql_error(),E_USER_ERROR);}
									} else {
										$query="SELECT `site_id`,`license` FROM `{$dbtable_prefix}user_sites` WHERE `fk_user_id`=".$real_user['user_id']." LIMIT 1";
										if (!($res=@mysql_query($query))) {trigger_error(mysql_error(),E_USER_ERROR);}
										if (mysql_num_rows($res)) {
									...
									// now insert all bought products into db
									$query="INSERT INTO `{$dbtable_prefix}user_products` (`fk_prod_id`,`fk_site_id`,`processor`,`orderno`,`date_purchased`,`license`,`license_md5`) VALUES ";
									for ($i=0;isset($prods[$i]);++$i) {
										$query.="(".$prods[$i].",".$input['site_id'].",'twocheckout','".$input['x_trans_id']."',now()";
									...
									// now show the thank you page with all required details.
									if (!$this->is_fraud) {
										...
										$tpl->drop_var('output');
// DON'T FORGET TO SEND THE EMAIL ALSO!!!!!!!
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
									} else {
					...
					require_once _BASEPATH_.'/includes/classes/log_error.class.php';
					new log_error(array('module_name'=>get_class($this),'text'=>'Invalid hash code received after payment: '.array2qs($input)));
				}
			} else {
