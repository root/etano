...
class widget_latest_members extends icontent_widget {
	...
	function _content() {
		...
		$query="SELECT a.`fk_user_id`,b.`last_activity` FROM `{$dbtable_prefix}user_profiles` a LEFT JOIN `{$dbtable_prefix}online` b ON a.`fk_user_id`=b.`fk_user_id` WHERE a.`_photo`<>'' AND a.`del`=0 ORDER BY a.`date_added` DESC LIMIT 4";
		...
		if (!empty($user_ids)) {
			...
			$loop=$user_cache->get_cache_beta($user_ids,$onlines,'details_gallery','tpl');
			$loop[0]['class']='first';
