			...
			if (empty($config['manual_com_approval'])) {
				...
				if (mysql_num_rows($res)) {
					...
					if (isset($_SESSION['user']['user_id']) && $_SESSION['user']['user_id']!=$notification['fk_user_id']) {
						...
						$notification['message_body']=sprintf('%1$s posted a comment on one of your photos.<br><a class="content-link simple" href="photo_view.php?photo_id=%2$s#comm%3$s">Click here</a> to view the comment',$_SESSION['user']['user'],$input['fk_photo_id'],$input['comment_id']);
