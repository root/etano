		...
		if (!empty($input['nconn_id'])) {
			if (isset($_POST['btn_accept'])) {
				...
				while ($rsrow=mysql_fetch_assoc($res)) {
					$query="INSERT IGNORE INTO `{$dbtable_prefix}user_networks` SET `fk_user_id`='".$_SESSION[_LICENSE_KEY_]['user']['user_id']."',`fk_net_id`=".$rsrow['fk_net_id'].",`fk_user_id_other`=".$rsrow['fk_user_id'].",`nconn_status`=1";
