...
if (is_file(_BASEPATH_.'/events/processors/blog_comments.php')) {
	include_once _BASEPATH_.'/events/processors/blog_comments.php';
}
	...
	if (!$error) {
		...
		if (!empty($input['comment_id'])) {
			...
			if (!empty($_SESSION[_LICENSE_KEY_]['user']['user_id'])) {
				$input['comment'].="\n\nLast edited by ".$_SESSION[_LICENSE_KEY_]['user']['user'].' on '.gmdate('Y-m-d H:i').' GMT';	// translate
			...
			$now=gmdate('YmdHis');
			$query="INSERT INTO `{$dbtable_prefix}blog_comments` SET `_user`='".$_SESSION[_LICENSE_KEY_]['user']['user']."',`date_posted`='$now',`last_changed`='$now'";
			...
			if (empty($config['manual_com_approval'])) {
				...
				if (mysql_num_rows($res)) {
					...
					if (!empty($_SESSION[_LICENSE_KEY_]['user']['user_id']) && $_SESSION[_LICENSE_KEY_]['user']['user_id']!=$notification['fk_user_id']) {
						...
						$notification['message_body']=sprintf('%1$s posted a comment on one of your blog posts.<br><a class="content-link simple" href="blog_post_view.php?pid=%2$s#comm%3$s">Click here</a> to view the comment',$_SESSION[_LICENSE_KEY_]['user']['user'],$input['fk_parent_id'],$input['comment_id']);
