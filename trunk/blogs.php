...
if (isset($_SESSION['user']['user_id'])) {
	$blog_stats=get_module_stats(2,$_SESSION['user']['user_id']);
}
...
$latest_blogs=sanitize_and_format($latest_blogs,TYPE_STRING,$__html2format[TEXT_DB2DISPLAY]);

$featured_blogs=array();

$tpl->set_file('content','blogs.html');
if (!empty($blog_stats)) {
	$tpl->set_var('blog_stats',$blog_stats);
}
$tpl->set_loop('latest_blogs',$latest_blogs);
$tpl->set_loop('featured_blogs',$featured_blogs);
$tpl->process('content','content',TPL_LOOP | TPL_NOLOOP | TPL_OPTIONAL | TPL_OPTLOOP);
$tpl->drop_loop('latest_blogs');
$tpl->drop_loop('featured_blogs');
