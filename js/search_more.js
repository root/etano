...
function update_location(data) {
	if (data!=null && data!='') {
		...
		document.getElementById('row_'+str_field+'state').className='search_row invisible';
		document.getElementById('row_'+str_field+'city').className='search_row invisible';
		document.getElementById('row_'+str_field+'zip').className='search_row invisible';
		for (i=0;i<toshow.length;i++) {
			document.getElementById('row_'+toshow[i]).className='search_row visible';
		}
		if (allopts.length>3) {
