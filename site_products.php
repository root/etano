...
$loop=array();
$query="SELECT b.`prod_id`,b.`prod_name`,b.`version` FROM `user_products` a,`products` b WHERE a.`fk_prod_id`=b.`prod_id` AND a.`fk_site_id`=".$output['site_id']." AND b.`is_visible`=1 AND a.`fk_user_id`=".$_SESSION[_LICENSE_KEY_]['user']['user_id'];
if (!($res=@mysql_query($query))) {trigger_error(mysql_error(),E_USER_ERROR);}
