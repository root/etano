	...
	if (!$error) {
		switch ($input['t']) {
			...
			case AMTPL_REJECT_COMM:
				...
				$query="UPDATE $table SET `status`='".STAT_EDIT."',`last_changed`='".gmdate('YmdHis')."',`reject_reason`='".$input['reject_reason']."' WHERE `comment_id`='".$input['id']."'";
				if (!($res=@mysql_query($query))) {trigger_error(mysql_error(),E_USER_ERROR);}
				if (!empty($input['send_email'])) {
					$query="SELECT b.`email` FROM $table a,".USER_ACCOUNTS_TABLE." b WHERE a.`fk_user_id`=b.`".USER_ACCOUNT_ID."` AND a.`comment_id`='".$input['id']."'";
					if (!($res=@mysql_query($query))) {trigger_error(mysql_error(),E_USER_ERROR);}
					if (mysql_num_rows($res)) {
						$ok=queue_or_send_email(array(mysql_result($res,0,0)),array('subject'=>$_POST['reason_title'],'message_body'=>$_POST['reject_reason']));
					}
