...
if (!$error) {
	...
	if (!$error) {
		$query="INSERT IGNORE INTO `{$dbtable_prefix}user_networks` SET `fk_user_id`='".$_SESSION['user']['user_id']."',`fk_net_id`='".$input['net_id']."',`fk_user_id_other`='".$input['uid']."',`nconn_status`='".(1-(int)$is_bidi)."'";
		if (!($res=@mysql_query($query))) {trigger_error(mysql_error(),E_USER_ERROR);}
		...
		if (!empty($is_bidi)) {
			...
			$request['message_body']='<a class="content-link simple" href="friendship_requests.php">Click here</a> to see all friendship requests';
			$request['message_type']=MESS_MESS;
