...
Etano
===============================================================================
File:                       ajax/get_photo_comment.php
$Revision$
...
if (!empty($_SESSION[_LICENSE_KEY_]['user']['user_id']) && !empty($_POST['comment_id'])) {
	...
	$query="SELECT `comment_id`,`comment` FROM `{$dbtable_prefix}photo_comments` WHERE `comment_id`=$comment_id AND `fk_user_id`='".$_SESSION[_LICENSE_KEY_]['user']['user_id']."'";
